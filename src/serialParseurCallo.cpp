   ...
   /// et de faire le setup des pins Arduino ou autre composant
{
  ...
  _seprateurCMD=32;     // ici le séparateur est fixé à 32 entre une simple commande et une commande avec séparateur
...
void serialParseurCallo::choixCommande(bool debug)
  {                                                                     /// $ + QUALIFICATEUR + , + NuméroDeCommande + , + PARAMETRE + , + <CR> + <LF>
   /* Récupération commande + MAJ des drapeaux */
     _valString += (char)Serial.read();_valString="";                    // on élimine le caratère "," après le qualificateur
     ...
     _valString += (char)Serial.read();                                  // on récupére le  3ème caractère de la commande
...
void serialParseurCallo::parametre(byte subFrameLenght)
  {
                                                                        /// $ + QUALIFICATEUR + , + NuméroDeCommande + , + PARAMETRE + , + <CR> + <LF>
           _parametre += (char)Serial.read();_parametre="";              // on élimine le caratère "," au départ de PARAMETRE
...
void serialParseurCallo::printStatut()
  {
	...
	*/
    for ( i=0;i<nb;i++)
        {
		    j++;
			if (j>6) {Serial.println("");j=0;}
			Serial.print(i);Serial.print(":[");
			Serial.print(_statut[i]);Serial.print("]");
            Serial.print("--");
